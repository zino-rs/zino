// 重新导出公共类型，保持向后兼容性
pub use super::config::{CachePolicy, NodeConfig, NodeParamTypes, RetryPolicy};
pub use super::error::{WorkflowError, WorkflowResult};
pub use super::executor::WorkflowExecutor;
pub use super::graph::{BranchSpec, CompiledStateGraph, StateGraph, StateNodeSpec};
...
pub use super::state::{Channel, ExecutionTask, StateValue, WorkflowState};
pub use super::traits::{
    BranchPath, BranchResult, ChannelWriter, NodeContext, NodeStore, Runtime, StateNode,
};
...
mod tests {
    ...
    async fn test_simple_workflow() {
        ...
        // 创建节点
        let node1 = StateNodeSpec::new(node!("process", |state| {
            println!("处理数据: {:?}", state);
        ...
        channels.insert(
            "input".to_string(),
            Channel::new_last_value(StateValue::Null),
        );
        ...
        input.insert(
            "input".to_string(),
            StateValue::String("测试数据".to_string()),
        );
    ...
    async fn test_complex_workflow() {
        ...
        // 1. 数据预处理节点
        let preprocess_node = StateNodeSpec::new(node!("preprocess", |state| {
        ...
        // 2. 数据验证节点
        let validate_node = StateNodeSpec::new(node!("validate", |state| {
        ...
        // 3. 分支节点 - 根据验证结果决定路径
        let branch_node = StateNodeSpec::new(node!("branch", |state| {
        ...
        // 4. 成功处理节点
        let success_node = StateNodeSpec::new(node!("success", |state| {
            println!("🎉 成功节点: 处理有效数据");
        ...
        // 5. 错误处理节点
        let error_node = StateNodeSpec::new(node!("error", |_state| {
            println!("❌ 错误节点: 处理无效数据");
        ...
        // 6. 最终汇总节点
        let summary_node = StateNodeSpec::new(node!("summary", |state| {
            println!("📊 汇总节点: 生成最终报告");
            let mut summary = HashMap::new();
            summary.insert(
                "status".to_string(),
                StateValue::String("completed".to_string()),
            );
            summary.insert(
                "timestamp".to_string(),
                StateValue::Number(chrono::Utc::now().timestamp() as f64),
            );
        ...
        input1.insert(
            "raw_data".to_string(),
            StateValue::String("Hello World".to_string()),
        );
    ...
    async fn test_async_workflow() {
        ...
        // 异步处理节点
        let async_node = StateNodeSpec::new(node!("async_process", async |state| {
            println!("⏳ 异步节点: 开始异步处理");
        ...
        input.insert(
            "input".to_string(),
            StateValue::String("异步测试数据".to_string()),
        );
    ...
    async fn test_simple_node_creation() {
        ...
        // 使用最简单的语法：字符串 + 函数名
        let process_node = StateNodeSpec::new(node!("process", process_data));

        let validate_node = StateNodeSpec::new(node!("validate", validate_data));
        ...
        // 设置边连接
        graph.add_edge("process".to_string(), "validate".to_string());
    ...
    async fn test_number_processing_workflow() {
        ...
        // 1. 输入节点 - 获取输入数据
        let input_node = StateNodeSpec::new(node!("input", |state| {
            println!("📥 输入节点: 获取输入数据");
            let state: &StateValue = &state;
            if let Some(data) = state.as_string() {
        ...
        // 2. 乘法节点 - 乘以一个数字
        let multiply_node = StateNodeSpec::new(node!("multiply", |state| {
            println!("✖️ 乘法节点: 乘以数字");
            let state: &StateValue = &state;
        ...
        // 4. 翻倍节点 - 数值翻倍
        let double_node = StateNodeSpec::new(node!("double", |state| {
            println!("🔄 翻倍节点: 数值翻倍");
            let state: &StateValue = &state;
            if let Some(number) = state.as_number() {
        ...
        // 5. 减半节点 - 数值减半
        let half_node = StateNodeSpec::new(node!("half", |state| {
            println!("➗ 减半节点: 数值减半");
            let state: &StateValue = &state;
            if let Some(number) = state.as_number() {
                let result = number / 2.0;
        ...
        // 6. 输出节点 - 打印最终结果
        let output_node = StateNodeSpec::new(node!("output", |state| {
            println!("📤 输出节点: 打印最终结果");
            let state: &StateValue = &state;
            if let Some(number) = state.as_number() {
    ...
    async fn test_complex_branching_workflow() {
        ...
        // 3. 分支节点1 - 判断正负
        let branch1_node = StateNodeSpec::new(node!("branch1", |state| {
            println!("🔀 分支节点1: 判断正负");
            let state: &StateValue = &state;
            if let Some(number) = state.as_number() {
                if number >= 0.0 {
        ...
        // 4. 正数处理节点
        let positive_node = StateNodeSpec::new(node!("positive", |state| {
            println!("➕ 正数处理节点: 处理正数");
            let state: &StateValue = &state;
            if let Some(number) = state.as_number() {
        ...
        // 5. 负数处理节点
        let negative_node = StateNodeSpec::new(node!("negative", |state| {
            println!("➖ 负数处理节点: 处理负数");
            let state: &StateValue = &state;
            if let Some(number) = state.as_number() {
                let result = number - 100.0;
        ...
        // 6. 分支节点2 - 判断大小
        let branch2_node = StateNodeSpec::new(node!("branch2", |state| {
        ...
        // 7. 大数处理节点
        let large_node = StateNodeSpec::new(node!("large", |state| {
            println!("🔢 大数处理节点: 处理大数");
            let state: &StateValue = &state;
            if let Some(number) = state.as_number() {
        ...
        // 8. 小数处理节点
        let small_node = StateNodeSpec::new(node!("small", |state| {
            println!("🔢 小数处理节点: 处理小数");
            let state: &StateValue = &state;
            if let Some(number) = state.as_number() {
                let result = number / 10.0;
        ...
        // 9. 最终汇聚节点
        let final_node = StateNodeSpec::new(node!("final", |state| {
            println!("🎯 最终节点: 生成最终结果");
            let state: &StateValue = &state;
            if let Some(number) = state.as_number() {
                let result = format!("最终结果: {}", number);
        ...
        if let Some(StateValue::String(output)) = result1.get("final_result") {
            assert!(
                output.contains("1200"),
                "场景1应该输出1200 (10→20→120→1200)"
            );
        }
    ...
    async fn test_edge_cases_workflow() {
        ...
        // 1. 边界值测试节点
        let boundary_node = StateNodeSpec::new(node!("boundary", |state| {
            println!("🔍 边界值测试节点");
            let state: &StateValue = &state;
            if let Some(number) = state.as_number() {
                if number == 0.0 {
        ...
        // 2. 错误处理节点
        let error_node = StateNodeSpec::new(node!("error_handler", |state| {
            println!("❌ 错误处理节点");
            let state: &StateValue = &state;
            match state {
                StateValue::Null => {
        ...
        // 3. 最终结果节点
        let result_node = StateNodeSpec::new(node!("result", |state| {
            println!("📊 结果节点");
            let state: &StateValue = &state;
