...
pub(crate) async fn sse_handler(
    ...
    let session_id = subscription.session_id().map(|s| s.to_string());
    let source = subscription.source().map(|s| s.to_string());
    let topic = subscription.topic().map(|s| s.to_string());
    ...
    let stream = channel.into_stream().filter_map(move |event| {
        ...
        if session_id.is_none() || session_id.as_deref() != event_session_id {
            ...
            if source.as_ref().filter(|&s| s != event_source).is_none() {
                let event_topic = event.topic();
                if topic.as_ref().filter(|&t| t != event_topic).is_none() {
